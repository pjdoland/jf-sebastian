...
|----|-------|-------|-----|-----|
| M9 | Local/Offline Mode With Fallback Models | 7/7 | M, S, L, C, A, P, R | none |
| S2 | Local Streaming TTS with Sub-200ms First-Audio | 7/7 | M, S, L, C, A, P, R | none |
| P1 | Pluggable STT Backend Architecture | 7/7 | M, S, L, C, A, P, R | none |
| P2 | Pluggable LLM Backend (Ollama / llama.cpp) | 7/7 | M, S, L, C, A, P, R | none |
| P3 | Pluggable TTS Backend (Piper / Coqui) | 7/7 | M, S, L, C, A, P, R | none |
| P8 | Local Weather & Context Provider | 7/7 | M, S, L, C, A, P, R | none |
| R3 | OTA Personality & Wake-Word Model Updates | 7/7 | M, S, L, C, A, P, R | none |
...
> **Status update:** all three Tier-1 items have since shipped (in 2.4.0): the pluggable
...
>
> Two features that were never on this list have also shipped: Spotify playback control
> (2.7.0) and Philips Hue light control (2.8.0). Both are optional LLM tool providers built
> on the shared base in `jf_sebastian/modules/tool_provider.py`. See
> [docs/SPOTIFY_SETUP.md](docs/SPOTIFY_SETUP.md), [docs/HUE_SETUP.md](docs/HUE_SETUP.md),
...
**Status: Shipped.** `utils/weather.py` and `utils/news.py` provide pluggable provider interfaces (wttr, Home Assistant, manual, RSS, Hacker News) selected via `WEATHER_PROVIDER` / `NEWS_PROVIDER`. A dedicated local weather-station adapter was not built.
...
**Status: Shipped (time-based events).** `jf_sebastian/modules/scheduler.py` (`ProactiveScheduler`) fires events from a per-personality `scheduled_events.yaml` (`HH:MM`, `HH:MM weekdays|weekends|mon,wed`, `HH:MM YYYY-MM-DD`; `say:` or `prompt:`), only when the state machine is IDLE, with quiet hours and a `SCHEDULER_ENABLED` toggle. It is a small built-in scheduler thread rather than APScheduler. Webhook/HomeKit triggers were not built and remain open.
...
reactive: it only wakes on the wake word.
